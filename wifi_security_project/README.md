...
8. [Вопросы лабораторной](#вопросы-лабораторной)
9. [Технические детали протоколов](#технические-детали-протоколов)
...
| Протокол | Шифр | Уязвимость | Скорость атаки |
|---|---|---|---|
| **WEP** | RC4 + CRC-32 | Короткий IV (24 бит), слабые ключи RC4 | ~100 000–1 000 000 кл/с |
| **WPA/TKIP** | RC4 + HMAC-MD5 | Устаревший, TKIP deprecated | ~10 000–50 000 чек/с |
| **WPA2/CCMP** | AES-128 + HMAC-SHA1 | Только слабые пароли | ~300–2 000 пар/с |
...
```bash
unzip wifi_security_project.zip
...
```
scapy>=2.5.0      # для capture_real.py (захват с интерфейса)
tqdm>=4.65.0      # прогресс-бары в crackers
colorama>=0.4.6   # цветной вывод на Windows
tabulate>=0.9.0   # итоговая таблица бенчмарка
```

> Если нужны только `src/` и `lab/` (без реального захвата), достаточно:
> ```bash
...
Выводит таблицу: длина пароля × набор символов → количество комбинаций → время перебора на Python и GPU. Помогает ответить на вопрос Q3 в лабораторном отчёте.
...
Файл `lab/aes_crack.py` содержит пять вопросов для отчёта. Краткие ответы:

**Q1. Почему нельзя атаковать AES напрямую?**  
AES-128 имеет 2¹²⁸ возможных ключей. Лучшая известная атака (связанные ключи) требует 2¹²⁶·¹ операций — непрактично. Атакуют пароль (WPA2-Personal), не ключ шифрования.

**Q2. Роль PBKDF2 в WPA2?**  
Key stretching: замедляет перебор с ~10⁸ до ~10³ попыток/с на CPU. Делает словарные атаки дорогостоящими даже при GPU.

**Q3. Сколько дней GPU взламывает 12-символьный пароль?**  
`62^12 / 10^6 / 86400 ≈ 3 700 лет` (GPU hashcat ~10⁶ пар/с, 62-символьный charset).

**Q4. WPA2-Personal vs WPA2-Enterprise?**  
Personal: один PSK на всю сеть, уязвим к словарным атакам.  
Enterprise: каждый пользователь аутентифицируется через 802.1X/RADIUS, у каждого сеанса уникальный PTK — PSK не существует, словарная атака невозможна.

**Q5. Почему SSID влияет на безопасность?**  
PMK = PBKDF2(пароль, **SSID**). Rainbow-таблицы предвычисляются под конкретный SSID. Стандартные SSID (HomeNetwork, linksys) имеют готовые таблицы. Уникальный SSID делает rainbow-таблицы бесполезными.
